...
# load data
x_train, y_train = load_data("twitter.training.csv", sample_ratio=1, n_class=2)
x_test, y_test = load_data("twitter.test.csv", sample_ratio=1, n_class=2)
...
x_train, x_dev, y_train, y_dev, train_size, dev_size = \
    split_dataset(x_train, y_train, 0.001)
...
with graph.as_default():
    ...
    rnn_outputs, _ = tf.compat.v1.nn.dynamic_rnn(BasicLSTMCell(HIDDEN_SIZE), batch_embedded, dtype=tf.float32)
    ...
    W = tf.Variable(tf.random.truncated_normal([shape[1], MAX_LABEL], stddev=0.1))
